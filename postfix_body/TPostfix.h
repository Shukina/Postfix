...
public:
	TPostfix(string _str)
	{
		...
		postfix = "Not Exist";
		IsCorrect(infix);
		CorrectWithUnarMinus();
		TOperations = new string[8];//операции
		TOperations[0] = '+';
		TOperations[1] = '-';
		TOperations[2] = '*';
		TOperations[3] = '/';
	...
	bool IsCorrect(string &str) //проверка на корректность введённой строки
	{
		...
		{
			now = str[i];
			if (!((((now > 39) && (now < 58)) || (now > 64) && (now < 91)) || ((now > 96) && (now < 123))))//операции+числа
			{
